...
// This page can be seen built at https://docs.couchbase.com/java-analytics-sdk/current/project-docs/metadoc-about-these-sdk-docs.html
...
The Analytics SDKs simply interact with Enterprise Analytics, not Couchbase Server and its various services --
but some of that complexity spills across into the Analytics SDKs and their documentation.
...
Large as this docset is, it can only cover a subset of the complete Analytics SDK API.
...
The heart of the Analytics Cluster -- real-time OLAP querying, with Couchbase's familiar {sqlpp} query language.
...
TIP: You can also file JIRA tickets against these docs by clicking the *Leave Additional Feedback?* link on most pages in the docs.
