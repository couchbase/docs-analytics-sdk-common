...
:page-toclevels: 2
:description: Troubleshoot network connection problems with SDK doctor.
...
You may be hosting your Enterprise Analytics cluster in a private cloud or remote data center, with development on a local laptop;
this can lead to timeouts not seen in a production environment (where the database and app are hosted in the same LAN-like environment),
and that can confuse the debugging of problems.
SDK doctor is a powerful but simple tool to help diagnose connection problems.
